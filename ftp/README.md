...
====================
This image is intended to make easier to deploy a plain FTP server with
[FXP](https://en.wikipedia.org/wiki/File_eXchange_Protocol) enabled.

In this repository you can find both the `Dockerfile` to build the image,
and a `docker-compose.yml` file to deploy it.

You may use our pre-built image `gitlab-registry.cern.ch/fts/ready-to-go/ftp:latest`,
